<x-layouts.admin>
    ...
    <x-slot name="favorite"
        title="{{ trans('general.title.new', ['type' => trans_choice('general.investments', 1)]) }}"
        icon="savings"
    ...
    <x-slot name="content">
        <x-form.container>
            <x-form id="investment" route="investments.store">
                <x-form.section>
                    <x-slot name="head">
                        <x-form.section.head title="{{ trans('general.general') }}" description="{{ trans('investments.form_description.general') }}" />
                    </x-slot>

                    <x-slot name="body">
                        <x-form.group.text name="contact_name" label="{{ trans('investments.contact_name') }}" />
                ...
                <x-form.section>
                    <x-slot name="head">
                        <x-form.section.head title="{{ trans_choice('general.others', 1) }}" description="{{ trans('investments.form_description.other') }}" />
                    </x-slot>
